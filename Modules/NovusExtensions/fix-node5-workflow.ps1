...
try {
    ...
    # Find the Claude AI Analysis node and update it
    $claudeNode = $workflow.nodes | Where-Object { $_.name -eq "Claude AI Analysis" }

    if ($claudeNode) {
        Write-Host "Updating Claude AI Analysis node..." -ForegroundColor Yellow

        # Change the jsonBody parameter to only include valid Claude API fields
        $claudeNode.parameters.jsonBody = "={{ JSON.stringify({ model: `$json.model, max_tokens: `$json.max_tokens, temperature: `$json.temperature, system: `$json.system, messages: `$json.messages }) }}"

        Write-Host "New jsonBody parameter:" -ForegroundColor Gray
        Write-Host $claudeNode.parameters.jsonBody -ForegroundColor Gray
        ...
        Write-Host "Could not find 'Claude AI Analysis' node" -ForegroundColor Red
    }
